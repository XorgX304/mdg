...
def generate():
    ...
    awk_generated = list(
        ...
        if post_data.get(header) in CONFIG['options']['awk_generated'])

    # Extract specific data types options
    options_dict = {
        key: post_data.get(key)
        for key in headers for option in CONFIG['options']['data_type_options']
        if option in key
    }
    ...
    if not is_csv(file_type):
        filename = file_conversion(file_type, filename, options_dict, headers,
                                   post_data)
...
def write_awk_generated(headers, awk_generated, post_data, num_rows, filename,
                        options):
    """
    ...
    os.system(
        awk.create_awk_statement(post_data, headers, num_rows, filename,
                                 options))
...
def write_python_generated(filename, post_data, python_generated, delimiter,
                           options):
...
def convert_to_xml(filename, options):
    ...
    os.system(CONFIG['conversion']['xml'].format(filename, root_node,
                                                 record_node))
    return xml_file
...
def convert_to_sql(filename, options, headers, post_data):
    ...
    table_name = options.get(CONFIG['options']['file_type_options'][0])

    # Get table name from options
    ...
    with open(sql_file, 'w') as file:
        ...
        if options.get(CONFIG['options']['file_type_options'][1]) == 'true':
            ...
            file.write(CONFIG['sql']['create'].format(
                table_name, sql_table_creation_string))
...
def compress_file(filename):
    ...
    with open(filename, 'rb') as in_file:
        with gzip.open(filename + CONFIG['extensions']['gzip'],
                       'wb') as out_file:
...
def check_request_validity(num_rows, headers):
    ...
    return all((validate_num_rows(num_rows), validate_header_length(headers),
                bad_header_names(headers)))
...
def bad_header_names(headers):
    ...
    """
    for header in headers:
        if header.lower() in CONFIG['bad_col_names'] or any(
                c in FORBIDDEN_CHARS for c in header) or header[0].isdigit():
            return False
    return True
